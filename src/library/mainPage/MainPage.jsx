...
const MainPage = () => {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get("https://icebreak-backend.onrender.com/games")
      .then((response) => {
  ...
  const drinkingGames = gameData.filter((game) =>
    game.tags.includes("drinking")
  );
      ...
      {!loading && (
        <AnimatedPage>
          <div className="asd">
            ...
            <div className="category-headers">Funny games 😂</div>
            <div className="cards">
              {funnyGames.map((game) => (
                <GameCard key={game.name} game={game} />
              ))}
            </div>
            <div className="category-headers">Drinking Games 🍻</div>
            <div className="cards">
